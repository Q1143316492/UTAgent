"""golden-path：登录面板 WndLogin（L1 结构锚，非 L2 答案）。

挂子相对 WndLogin 子树；拼装前清理同名旧根。
"""
import json
import os
import sys
...
def _make_input_row(row_name, label_text, input_name):
    """账号/密码行：标签 + 输入；尺寸走 LayoutElement.preferred*。"""
    ...
    inp.AddComponent(CS.UnityEngine.UI.Image).color = CS.UnityEngine.Color(0.1, 0.1, 0.12, 1)
    inp.AddComponent(CS.TMPro.TMP_InputField)
    inp.GetComponent(CS.UnityEngine.RectTransform).sizeDelta = CS.UnityEngine.Vector2(0, 0)
    le_i = inp.AddComponent(CS.UnityEngine.UI.LayoutElement)
    le_i.preferredWidth = 240
    le_i.preferredHeight = 32
    le_i.flexibleWidth = 1
    return row


def _add_button(parent_go, btn_name, btn_label):
    ...
    scope.add_child(parent_go, btn, preferred_w=120, preferred_h=40)
...
def build_login_form():
    ...
    wnd_rt.sizeDelta = CS.UnityEngine.Vector2(420, 320)
    ...
    title_tmp.ForceMeshUpdate()
    le_title = title.AddComponent(CS.UnityEngine.UI.LayoutElement)
    le_title.minHeight = 36
    le_title.preferredHeight = 36

    scope.add_child(panel_body, _make_input_row("RowUser", "账号", "InputUser"), preferred_h=40)
    scope.add_child(panel_body, _make_input_row("RowPass", "密码", "InputPass"), preferred_h=40)
    ...
    for btn_name, btn_label in [("BtnLogin", "登录"), ("BtnCancel", "取消")]:
        _add_button(panel_buttons, btn_name, btn_label)
...
print(json.dumps(build_login_form(), ensure_ascii=False))
