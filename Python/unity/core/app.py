...
class App:
    ...
    def sync_runtime_modules(cls, force=False):
        """按 Python/ 与 Scripts/ 下 .py 的磁盘 mtime 决定是否刷新模块缓存。
        ...
        _, python_dir, scripts_dir = dirs
        latest = max(
            cls._max_py_mtime_under(python_dir),
    ...
    def _purge_editor_modules(cls):
        ...
        _, python_dir, scripts_dir = dirs
        python_root = cls._norm_path(python_dir)
        ...
        for name in list(sys.modules.keys()):
            ...
            if file_path:
                if cls._is_under(file_path, python_root) or cls._is_under(file_path, scripts_root):
                    del sys.modules[name]
                continue

            # 无 __file__ 的 namespace package（如仅 Python 根在 path 时 import agent）
            ...
            for entry in entries:
                if cls._is_under(entry, python_root) or cls._is_under(entry, scripts_root):
                    del sys.modules[name]
                    break
    ...
    def _load_class(cls, module_path, class_name):
        ...
        if assets_root is None:
            raise RuntimeError("sys.path 中未找到 UTAgent/Python，请先初始化引擎")
    ...
    def _find_utagent_dirs(cls):
        for entry in sys.path:
            ...
            if not normalized.endswith("/UTAgent/Python"):
                continue
            python_dir = os.path.normpath(entry)
            assets_root = os.path.dirname(os.path.dirname(python_dir))
            scripts_dir = os.path.join(assets_root, "UTAgent", "Scripts")
            return assets_root, python_dir, scripts_dir
        return None
