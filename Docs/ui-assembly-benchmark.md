...
**门禁：** 薄入口 `run_assert_ui_scene_health.py`（实现 `assert_ui_scene_health`；近零 rect；Layout 下须 childControl + preferred；scoped 仍报 Canvas 外孤儿）。health FAIL 时 harness **打回 Agent 限次纠偏**（见 `format_health_remediation_prompt.py` / `-RemediationMax`），不得停在「无 export」。  
...
| E13 | 按需 | health 全量扫描入口 | `run_assert_ui_scene_health.py` | ✅ | 2026-07-19 | 挂在 L2/导出；薄入口 2026-07-22 |
