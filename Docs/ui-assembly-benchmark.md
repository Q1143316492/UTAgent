...
> **本文件是 UI 拼装 L0/L1/L2 验收用例表与结果列的真源（回归用，不是路线图）。**
> **Agent 评测 / 核心循环学习路线**见 `Docs/ut-agent/21-agent-core-loop-and-eval.md`（本表为默认实践套件）。
> **产品 CLI 下一步**见 `Docs/ut-agent/18-next-plan-cli-and-surface.md`；对齐收官见 `17-next-plan-post-alignment.md`。
> 各 change 的 `verification.md` MUST NOT 重新定义用例 ID，仅引用本表表项。
> 改 UI 相关代码后，跑 `Tools/ui-benchmark/run_benchmark.ps1` 回归。
> 对齐设计基准见 `Docs/ut-agent/16-scene-inspection-and-cursor-workflow.md`。
...
| E02 | 面板：`Wnd*/PanelBody` + VLG + `TxtTitle` + `BtnSubmit`；`colors` 三档 | golden_path_layout_panel | ✅ | 2026-07-14 | editor-ui-slim-templates |
...
| E06 | `add_to_layout`：挂子后 `parent==PanelBody` 且子 `anchorMin` 未被原语改 | （inline 测试） | ✅ pre=post=[0.5,0.5] | 2026-07-15 | editor-ui-layout-primitives |
...
| C11 | 强制漏设 childControl 的 VLG（禁 loadSkill、必须真实 exec） | before-exec `layout-control → inject reminder` | ≤4 | ✅ layout-control inject | 2026-07-17 | （直接落地，无独立 change） |
| C12 | 强制 `print('A'*12000)`（禁 loadSkill、必须真实 exec） | after-tool `truncate … → rewrite` | ≤4 | ✅ truncate rewrite（12033 chars） | 2026-07-17 | agent-after-tool-truncate |
| C13 | 连续 ≥3 次 `find_objects('BtnDoesNotExistC13')`（禁创建；须开 noProgress） | after-tool `no-progress … → inject reminder` | ≤8 | ✅ no-progress inject（streak=3） | 2026-07-17 | agent-after-tool-no-progress |
...
| after-tool | `[HH:mm:ss] after-tool` + 缩行 `<domain>[, <detail>] → <action>` | `after_tool_decisions[]` |
...
domain ∈ {non-ui, ui-domain, debug-domain, code-too-long, heavy-reflection, layout-control}。
