...
3. 一次或多次：
   ...
   - utagent screenshot [--name Wnd…]  # 可选：PNG → Out/screenshots/；可按 UI 节点裁切
...
| `screenshot` | 目检辅助：PNG 落盘；`--name` 按 UI 节点裁切 |
| `log tail` / `errors` | 诊断 |
...
# HUD / 面板：按节点裁切
./Assets/UTAgent/Tools/utagent-cli/utagent.ps1 screenshot --view game --name WndStardewHud --width 1920 --height 1080
```
